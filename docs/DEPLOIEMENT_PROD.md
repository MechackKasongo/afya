...
Internet → Ingress (TLS) → web (SPA) + api (gateway Nginx)
                              ↓
                            bff → 7 microservices → PostgreSQL (7 bases)
                            clinical → S3 / MinIO
Prometheus ← scrape /actuator/prometheus
```

Composants déployés par le chart Helm :

| Pod | Image | Port |
|-----|-------|------|
| `identity` | `afya-identity` | 8081 |
| `catalog` | `afya-catalog` | 8082 |
| `patient` | `afya-patient` | 8083 |
| `care-entry` | `afya-care-entry` | 8084 |
| `stay` | `afya-stay` | 8085 |
| `clinical` | `afya-clinical` | 8086 |
| `audit` | `afya-audit` | 8087 |
| `bff` | `afya-bff` | 8080 |
| `api` | `afya-gateway` | 80 |
| `web` | `afya-web` | 80 |
...
- **PostgreSQL 16** managé (RDS, Cloud SQL, Azure Database…) avec **7 bases** :
...
2. Le workflow **CD** (`.github/workflows/cd.yml`) :
   - exécute `mvn verify` implicitement via build Docker multi-stage
   - publie 10 images sur `ghcr.io/<org>/afya-*:<tag>`
...
for module in auth-service:user-service:hospital-service:patient-service:admission-service:medical-service:nursing-service:lab-service:report-service:audit-service:afya-bff; do
  ...
  docker build -f infra/docker/Dockerfile.spring --build-arg MAVEN_MODULE="$module" \
    -t "${REGISTRY}/afya-${name#afya-}:${TAG}" .
done

docker build -t "${REGISTRY}/afya-gateway:${TAG}" infra/gateway
docker build -t "${REGISTRY}/afya-web:${TAG}" frontend
```
...
kubectl -n afya-prod create secret generic afya-secrets \
  ...
  --from-literal=identity-db-password="..." \
  --from-literal=catalog-db-password="..." \
  --from-literal=patient-db-password="..." \
  --from-literal=care-entry-db-password="..." \
  --from-literal=stay-db-password="..." \
  --from-literal=clinical-db-password="..." \
...
helm upgrade --install afya-staging infra/k8s/helm/afya \
  ...
  --set databases.identity.host=afya-staging.xxxx.rds.amazonaws.com \
  --set databases.catalog.host=afya-staging.xxxx.rds.amazonaws.com \
  --set databases.patient.host=afya-staging.xxxx.rds.amazonaws.com \
  --set databases.care-entry.host=afya-staging.xxxx.rds.amazonaws.com \
  --set databases.stay.host=afya-staging.xxxx.rds.amazonaws.com \
  --set databases.clinical.host=afya-staging.xxxx.rds.amazonaws.com \
...
helm upgrade --install afya infra/k8s/helm/afya \
  ...
  --set databases.identity.host=afya-prod.xxxx.rds.amazonaws.com \
  # ... (mêmes --set pour chaque base et objectStore) \
...
# Smoke API (adapter GATEWAY_BASE)
GATEWAY_BASE=https://afya.example.com ./scripts/smoke-api.sh
...
- [ ] Images taguées et poussées sur GHCR
- [ ] Secret `afya-secrets` créé (aucune valeur dev)
- [ ] 7 bases PostgreSQL provisionnées + Flyway OK au premier démarrage
...
- [ ] Dashboard Grafana + alertes actives
- [ ] Sauvegarde/restauration BDD testée
- [ ] `/actuator` non accessible depuis Internet
...
| Symptôme | Action |
|----------|--------|
| Pod `CrashLoopBackOff` au démarrage | Logs : `kubectl logs -n afya-prod deploy/identity` — souvent secret JWT manquant ou valeur dev |
...
Ordre de redéploiement en cas de cascade : `audit` → `identity` → `catalog`/`patient` → `stay`/`care-entry` → `clinical` → `bff` → `api` → `web`.
